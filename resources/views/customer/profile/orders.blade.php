    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Order History</h1>
                    <p class="text-gray-600 mt-1 text-sm">View and track all your orders</p>
                </div>
                <a href="{{ route('products.index') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm text-center">
                    ...
                    @foreach($orders as $order)
                        <div class="p-4 sm:p-6">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
                                <div>
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900">Order #{{ $order->order_number ?? $order->id }}</h3>
                                ...
                                <div class="flex flex-wrap items-center gap-3 mb-4">
                                    @foreach($order->items->take(3) as $item)
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @endif
                                            <div class="min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate max-w-[120px] sm:max-w-none">{{ $item->product->name ?? 'Product' }}</p>
